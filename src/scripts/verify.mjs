...
const EXPECTED_SLOT = "skills-index:agent-skill";
...
function validatePackageJson(packageJson) {
  const errors = [];
  if (packageJson.name !== "@xuanzhen-tech/agent-skill-brick") errors.push("name must be @xuanzhen-tech/agent-skill-brick");
  ...
  if (packageJson.bin?.["agent-skill"] !== "./src/cli.mjs") errors.push("bin.agent-skill must point to ./src/cli.mjs");
...
async function validateDescriptorIfPresent(filePath) {
  ...
  if (descriptor.type !== "skills-index") throw new Error(`Invalid descriptor ${filePath}: type must be skills-index`);
  if (descriptor.slot !== EXPECTED_SLOT) throw new Error(`Invalid descriptor ${filePath}: slot must be ${EXPECTED_SLOT}`);
  ...
  assertNoSecretText(content, filePath);
...
async function validateRuntimeArtifactIfPresent() {
  ...
  for (const file of runtimeFiles) {
    ...
    if (
      ...
      normalized.includes("agent-tool") ||
  ...
  if (runtimeContract.schemaVersion !== "agent-skill.runtime.v1") {
    throw new Error("runtime-contract.json schemaVersion must be agent-skill.runtime.v1");
  }
  if (runtimeContract.artifactType !== "skills-index") {
    throw new Error("runtime-contract.json artifactType must be skills-index");
  }
  if (runtimeContract.command !== "agent-skill") {
    throw new Error("runtime-contract.json command must be agent-skill");
  }
...
async function assertRuntimeFilesDoNotContainSecrets(runtimeFiles) {
  for (const file of runtimeFiles) {
    ...
    assertNoSecretText(content, file);
  }
}
