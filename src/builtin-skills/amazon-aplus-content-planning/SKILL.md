...
读取上游时保留路径、版本、期间、使用字段、原始依据位置和限制。现有图片能否合法使用由用户或权利方确认，Agent 不自行推断版权。

### 禁止

- 本 Skill 不直接调用 `sif_mcp`、`sellersprite_mcp`、`sorftime_mcp`；只消费市场研究、关键词架构、质量审计、VOC 或公开 A+ 证据已交付的可追溯上游对象；
- 上游 MCP 证据须保留供应商与精确工具、原始结果位置、站点/ASIN/变体/期间、实际覆盖和原有限制；本包引用时保留上游文件与版本；
- 同类供应商证据仅在口径可比时共同影响模块优先级；不盲目平均，不隐藏冲突。供应商覆盖缺口、压缩或截断必须传递，不能写成三源一致；
- 供应商 Listing、A+、评论、关键词或 ASIN 证据只用于 brief/benchmark，不能补产品事实、合规、功效、权利或 A+ 发布资格；
...
每个模块目标、叙事与视觉 brief 都列出所使用的产品事实、VOC、关键词证据和品牌规则，并解释为何该模块适合承载这些信息、Agent 做了怎样的取舍、还缺什么资产或批准。上游供应商销量或流量只能作为受众与内容重点的背景，不能证明某模块会提高转化；上游文件、版本和原有限制必须保留。
...
- 资料冲突：事实、品牌或资产权利互相矛盾，暂停受影响模块；
- `blocked`：无法识别产品或核心事实；
- 请求涉及生图、上传、资格裁定或政策审批时，明确说明超出本 Skill 职责。
...
2. 确认产品事实、关键词、VOC 依据、资产路径/版本与权利情况真实存在；
3. 保留上游文件、版本、直接依据、数据期间、数值性质和使用限制；
4. 仅使用上游明确交付的字段，不从供应商展示块、图片存在标志、未返回字段或描述扩写 A+ 内容；
5. 把未查询、未返回、解析失败、资料缺失、来源冲突与来源明确给出的零值分开；
6. 上游合同、站点、ASIN/变体、期间、粒度、单位、定义、覆盖或版本不匹配时停止受影响模块，不直接调用 MCP 或其他来源补齐。
...
先选择一个已确认能够代表本产品身份的资产作为产品身份基准资产，并记录其名称和文件路径。它可以是用户确认的主图或其他权利清楚的基准图；没有合格锚点时，只能写待制作要求，不能假装已经锁定产品外观。所有模块共享一份视觉约束，至少记录产品外观、色彩、字体体系、光照方向和背景语言；不得从候选素材照搬未经当前运营核验的固定模块、像素、字号或安全区规则。

为每个资产写清：

- 当前资产名称、文件路径和服务模块；
- 产品身份基准资产的名称、文件路径，以及当前资产必须继承的共享视觉约束；
- 画面目的，而非只写“高级感”；
...
3. 子体特有属性是否误扩散到其他变体；
4. 每个模块是否仍忠实服务各自产品事实和沟通任务。

若某个模块失败，只把受影响的模块名称/顺序、资产路径/版本、失败项和返工约束交给视觉责任方；未受影响模块保持冻结。局部返工后仍需复核共享锚点，不能以整套无差别重做掩盖问题。
...
- 上游材料的字段、版本或口径不符合当前任务：停止受影响模块，列出具体缺口，不直接取数补齐。
- 请求涉及生图、上传、资格裁定、后台审批或持续监控时，停止相应分支并转交责任方。
...
3. `aplus-evidence-ledger.md`：来源路径、模块直接依据、Agent 取舍、限制和下一步。
...
- 没有图片生成、编辑、上传或审核通过承诺；
- 没有直接调用三个 MCP 或其他外部业务数据源；上游供应商证据未填补产品事实、合规、权利或发布资格；
