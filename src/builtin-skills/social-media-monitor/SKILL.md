...
description: "用于 Amazon 品牌站外舆情和趋势监控场景，追踪话题、竞品、达人内容、风险评论和需求信号，输出社媒监测摘要。"
version: 0.1.0
collection: ecosystem
displayName: "社媒监控"
platforms: ["amazon"]
sceneTags: ["customer-voice", "brand-compliance", "analytics-automation"]
searchTags: ["amazon", "customer-voice", "brand-compliance", "analytics-automation", "tiktok-shop"]
legacyEcosystemId: "nexscope-ai-ecommerce-skills-social-media-monitor"
originKind: "template"
---

# social media monitor：Amazon 监控与运营看板

面向 Amazon 跨境卖家的监控与运营看板知识沉淀，围绕 social-media-monitor 提供可执行分析流程、关键指标、输出模板和合规边界。

## 适用场景与边界

适用于 Amazon 卖家围绕 监控与运营看板 做诊断、规划和复盘。可处理单个 ASIN、父子变体、SKU 组合或类目节点层面的运营问题，并把 Listing、广告、库存、价格、评论和品牌资产放在同一个 Amazon 经营语境下判断。

边界：输出经营分析和执行建议，不替代 Seller Central 后台数据、Amazon 官方费用表、法律意见或平台最终裁定。

## 输入信息清单

- 主题输入：ASIN、关键词、API、广告报表、库存、价格、Buy Box、Listing 资产、采集频率、阈值、负责人和告警渠道。
...
3. 执行主题分析：定义基线和异常阈值；分级告警；把异常关联到广告、库存、价格、评论、类目节点或页面变更；复盘 MTTD、MTTR 和误报。
...
- 监控指标表、告警规则、状态页字段、异常复盘模板和负责人分派规则。
...
主题：social media monitor：Amazon 监控与运营看板
...
2. 围绕 监控与运营看板 建立证据表，区分必须立即处理的问题和可实验优化项。
