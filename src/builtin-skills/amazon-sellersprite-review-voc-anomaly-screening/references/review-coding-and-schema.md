<!--
文件功能：定义 VOC 专家需要的最小评论证据、主题编码、主题表和主题趋势数据结构。
...
`parent_theme, child_theme, theme_direction, mention_review_n, eligible_review_n, mention_rate, rate_status, asin_scope, analysis_period, codebook_version, coverage_status, excerpt_refs, evidence_ids, limitations`
...
不同 comparison level 各自保存分母。时间桶覆盖不足、codebook 变化或对象映射变化时中断趋势。
