...
6. 调用 `ecommerce_image_generate` 后保存 `batchId`，再用 `ecommerce_image_batch` 的 `status` 和 `waitMs` 轮询。不要把 `queued` 当成已完成。
7. 向用户呈递完成项，并保留每张图的 `assetId`、`versionId` 和 path。只有实际观察到图片时才执行视觉质量判断。
