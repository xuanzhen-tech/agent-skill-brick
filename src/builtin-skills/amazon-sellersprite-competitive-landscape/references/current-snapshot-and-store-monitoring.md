<!--
文件功能：定义当前 ASIN 比较快照和店铺新品观测所需的最小字段、日期语义、状态与图表数据。
...
`marketplace, asin, parent_asin, variation, seller_or_store, brand, category, title_or_product_type, current_price, currency, current_bsr, bsr_category, rating, review_count, variation_summary, listing_or_provider_date, captured_at, source_tool, evidence_id, field_status, limitation`

图表可使用 KPI 卡和横向比较表。不同币种、父子体、价格类型、BSR 类目或采集时点不可直接排序；缺失字段显示状态，不显示为零或空白优势。
