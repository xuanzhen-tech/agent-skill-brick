...
职责边界：提供内置利润包输入准备与独立复核口径，不提供动态税率、平台费率或会计法律结论；具体数值来自用户输入、内置包输出或受限 MCP 供应商探索性信号。
...
工作表中的每个数值都要记录：数值、是否已确认、来源与原始位置、截至时间和使用理由。

逐项说明输入情况：

- 用户确认或可复核事实：可以进入正式计算；
- SIF、SellerSprite、Sorftime 供应商信号或用户待确认假设：只能进入情景预览；
- 明确不适用：数值可以为 0，但必须写明理由；
- 缺少数值或依据：停止盈利计算，并列出缺失内容和责任人。

缺失不等于 0，也不能作为数值送入公式。MCP 供应商来源不能直接变成正式成本事实。只有所有关键输入都已确认，才可以形成正式盈利结论。

正式利润真相来自内置 `amazon-sku-profit-summary`。本文件公式只用于输入合同、解释和独立复核；复核不一致时停止交付并排查，不用本地结果覆盖内置包。

### MCP 供应商观察与缺口

MCP 预填与探索性门槛必须说明每次调用和实际消费字段是成功返回、未查询、未返回、解析失败、资料缺失还是来源冲突。以上情况都不是数值 0，不得补成 0 或进入公式；只有对应响应明确返回零且字段语义可确认时才可作为零证据。

原始记录保存供应商与精确工具、站点、对象、期间、原值、原始结果位置、覆盖限制和关键参数依据。请求 ID 只在真实返回且排错确实需要时保留；缺少参数依据或 schema 不支持时停止该源。
...
每个进入结果表的情景都必须记录用户确认及其依据；未获用户确认的情景只能留在待确认问题中，不得参与决策。
